...
class CardDetailViewTest(TestCase):
    def setUp(self) -> None:
        self.client = Client()
        self.card = Card.objects.create(number='0000000000000001', series='SA')
        self.url = reverse('card-detail')
    ...
    def test_context_data(self):
        order = Order.objects.create(card=self.card)
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
...
class CardGenerateForm(TestCase):
    ...
    def test_post_invalid_form(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'card_create.html')
        self.assertFormError(response, 'form', 'series',
                             'This field is required.')
